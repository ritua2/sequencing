...
Or with OpenMP for multi-core parallelism (worth using if your machine has more than 1 core — the sandbox I tested on didn't, but the QC/trim and alignment loops are both parallelized):
...
**Paired-end** (your case, with `SRR9336476`):
```bash
...
The pipeline expects **GTF**, not GFF3. Your annotation file was GFF3, so I converted it before testing. If you're working from the same Ensembl-style GFF3, you'll need to do the same — a minimal gene-level conversion:
